...
half :: Int -> Int
half p = if (p `mod` 20 == 0) then p `div` 2 else (p + 10) `div` 2

calc :: Int -> String -> Int
calc price str = case str of
    [_, 'p'] -> 0
    [p, 'w'] -> calc (half price) [p, 'n']
    "Cn"     -> half price
    "In"     -> half price
    "An"     -> price
    _        -> error "invalid"
...
solve :: String -> String
solve input = do
    (basePrice, ':':passengers) <- reads input
    let passengers'    = splitOn "," passengers
        adults         = filter ((== 'A') . head) passengers'
        children       = filter ((== 'C') . head) passengers'
        infants        = filter ((== 'I') . head) passengers'

        adultsPrices   = map (calc basePrice) adults
        childrenPrices = map (calc basePrice) children
        infantsPrices  = map (calc basePrice) infants
        infantsPrices' = drop (length adults * 2) $ (reverse . sort) infantsPrices

    show $ sum adultsPrices + sum childrenPrices + sum infantsPrices'
