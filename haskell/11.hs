-- オフラインリアルタイムどう書く第11回の問題
-- ビットあみだくじ
-- http://qiita.com/Nabetani/items/93cde1a6b7561426a3ac

-- 1つ以上のビットが連続して立っている部分の両サイドの数字を入れ替える、
-- ということを繰り返して答えを求めます。
-- リストでスワップを行うのは大変そうだったのでArray型を使ってみました。
...
type Lines = Array Int Int

swap :: (Lines) -> (Int, Int) -> Lines
swap ary (a, b) = ary // [(b, ary ! a), (a, ary ! b)]

hexToBools :: Char -> [Bool]
hexToBools hex = map (testBit (digitToInt hex)) [3, 2, 1, 0]

swapPairs :: [(Bool, Int)] -> [(Int, Int)]
swapPairs [] = []
swapPairs ((False, _):xs) = swapPairs xs
swapPairs ((True, i):xs)  = let len = length $ takeWhile fst xs
                            in (i, i + len + 1) : swapPairs (drop len xs)

parse :: String -> [(Int, Int)]
parse input = do
    hex <- splitOn "-" input
    swapPairs $ zip (concatMap hexToBools hex) [0..]

initLines :: Lines
initLines = listArray (0, 8) [0..8]

swapAll :: [(Int, Int)] -> Lines
swapAll swaps = foldl swap initLines swaps

solve :: String -> String
solve = (concatMap show) . elems . swapAll . parse
