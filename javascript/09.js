function half(price) {
    return (price % 20 === 0) ? price / 2 : (price + 10) / 2;
}

function calcPrice(price, person) {
    switch(true) {
        case person[1] === "p"  : return 0;
        case person[1] === "w"  : return calcPrice(half(price), person[0] + "n");
        case person    === "Cn" : return half(price);
        case person    === "In" : return half(price);
        default                 : return price;
    }
}

function solve(input) {
    var basePrice  = parseInt(input.split(":")[0]);
    var passengers = input.split(":")[1];

    var ap = (passengers.match(/A./g) || []).map(function(p) { return calcPrice(basePrice, p) });
    var cp = (passengers.match(/C./g) || []).map(function(p) { return calcPrice(basePrice, p) });
    var ip = (passengers.match(/I./g) || []).map(function(p) { return calcPrice(basePrice, p) });
    ip.sort(function(a, b) { return a - b });
    if(ap.length) {
        ip = ip.slice(0, -ap.length * 2);
    }
    return ap.concat(cp).concat(ip).reduce(function(a, b) { return a + b }).toString();
}
